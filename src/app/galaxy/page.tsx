...
export default function Galaxy() {
  ...
  const hqRoles = [
    { title: "标准制定者", desc: "构建资本准入与模型审计的终极标尺，确保整张星系图谱的无缝融合与代码统一。" },
    { title: "资源赋能者", desc: "自顶向下倾泻极核级产业资源、并购模型与大体量基石基金阵列。" },
    { title: "生态连接者", desc: "打破信息孤岛，撮合跨界共生，成为创新经济网络中的超级路由器。" },
    { title: "品牌塑造者", desc: "为矩阵内的标的公司打上代表了绝对信任与顶级专业度的白名单烙印。" },
    { title: "风险守护者", desc: "悬崖边的最后一道保险带，以极端悲观主义视角预判周期危机并对冲闭环。" },
  ];

  const regions = [
    { id: 1, name: "华南极核 / 深圳", label: "大湾区资本枢纽", coord: "bottom-[20%] right-[30%]" },
    { id: 2, name: "华东极核 / 上海", label: "长三角生态动脉", coord: "top-[40%] right-[25%]" },
    { id: 3, name: "西南极核 / 成都", label: "新质生产力跳板", coord: "bottom-[35%] left-[45%]" },
    { id: 4, name: "华北前哨 / 北京", label: "政商顶层架构矩阵", coord: "top-[25%] right-[35%]" },
  ];
    ...
    <PageTransition>
      <div ref={containerRef} className="bg-[#050505]">
        ...
        <Section className="bg-[#020202] !py-0 border-b border-white/5 flex flex-col lg:flex-row min-h-screen">
                  ...
                  {regions.map((region) => (
                    ...
                    >
                       ...
                       <div className={`transition-all duration-500 ${activeRegion === region.id ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                          <p className="text-white font-medium text-lg whitespace-nowrap drop-shadow-md">{region.name}</p>
                          <p className="text-[#D4AF37] text-xs font-bold tracking-widest uppercase">{region.label}</p>
                       </div>
                    </div>
                  ))}
               </div>
            ...
            {/* 右侧：俱乐部列表与微站入口 */}
            <div className="w-full lg:w-1/3 flex flex-col h-screen overflow-y-auto">
               <div className="p-10 lg:p-16 border-b border-white/10 sticky top-0 bg-[#050505]/90 backdrop-blur z-20">
                  <h2 className="text-3xl lg:text-4xl font-light text-[#ECECEC] mb-4">区域俱乐部阵列</h2>
                  <p className="text-gray-500">深入当地实业土壤的地推引擎。</p>
               </div>
               <div className="flex flex-col flex-1 divide-y divide-white/5 relative z-10 bg-[#070707]">
                 {regions.map((region) => (
                    <div 
                      key={region.id}
                      onMouseEnter={() => setActiveRegion(region.id)}
                    ...
                    >
                       <div className="flex justify-between items-start mb-8">
                         <h3 className={`text-2xl font-medium transition-colors ${activeRegion === region.id ? 'text-[#D4AF37]' : 'text-gray-300'}`}>{region.name}</h3>
                       ...
                       <p className="text-gray-500 mb-8 max-w-sm line-clamp-3">
                         独占区域生态高地，共享极核算力。负责执行从优质资产圈选到当地政商协同的基础底座运营。
                       </p>
                       {/* 微站占位链接 */}
        ...
        <Section className="bg-[#050505] !py-32">
           <div className="max-w-7xl mx-auto px-6 w-full text-center">
              ...
              <div className="text-4xl md:text-5xl font-light text-[#ECECEC] mb-6">超级协同网络</div>
              <p className="text-xl text-gray-500 font-light max-w-3xl mx-auto mb-20 leading-relaxed">
                建立在垂直授权体系之上的平权共识网络。我们与拥有同样审美准则的全球顶尖律所、审计与保荐机构背靠背作战。
              </p>
                 ...
                 {Array.from({ length: 15 }).map((_, idx) => (
                    ...
                    >
                       ...
                       <span className="text-white/20 font-bold uppercase tracking-widest text-sm group-hover:text-white/60 transition-colors">
                          Partner {String.fromCharCode(65 + idx)}
                       </span>
                    </div>
                 ))}
              </div>
        ...
        {/* 模块四：加入星系 CTA */}
        <Section className="bg-[#020202] !min-h-[80vh] flex flex-col justify-center items-center relative overflow-hidden">
           ...
           <div className="relative z-10 text-center max-w-4xl px-6">
              <h2 className="text-6xl md:text-8xl font-black text-[#ECECEC] tracking-tighter mb-12 drop-shadow-[0_0_40px_rgba(255,255,255,0.1)]">
                 占领极点，<br/>
                 <span className="text-[#D4AF37] italic font-serif font-light">点亮下一个星区。</span>
              </h2>
